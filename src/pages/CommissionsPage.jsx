      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 lg:gap-8 mb-12 sm:mb-20">
          {commissionCategories.map((cat, idx) => (
            <motion.div
              ...
              className="anime-card p-4 sm:p-6 bg-white flex flex-col justify-between"
            >
              <div className="space-y-3 sm:space-y-4">
                {/* Badge & Tier */}
                <div className="flex items-center justify-between">
                  <span className="text-xs font-display font-semibold px-2.5 py-0.5 sm:px-3 sm:py-1 rounded-full bg-indigo-50 text-indigo-600 border border-indigo-200/60">
                    {cat.badge}
                  </span>
                ...
                {/* Price Display */}
                <div className="py-2 sm:py-2.5 px-3 sm:px-3.5 rounded-xl bg-slate-50 border border-slate-200/60 space-y-0.5">
                  ...
                  {cat.isPlaceholder && (
                    <p className="text-[10px] sm:text-[11px] font-mono text-indigo-600/80 italic">
              ...
              <div className="pt-4 sm:pt-6">
                <motion.button
                  ...
                  className="btn-primary w-full py-2.5 sm:py-3 px-4 text-xs font-display font-semibold flex items-center justify-center gap-2 touch-manipulation"
                >
        ...
        <div className="pt-12 sm:pt-16 border-t border-slate-200/80">
          <div className="text-center max-w-2xl mx-auto space-y-2 mb-8 sm:mb-12">
            ...
            <h3 className="text-xl sm:text-2xl md:text-3xl font-display font-bold text-slate-900">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            {commissionSteps.map((step, idx) => (
              <motion.div
                ...
                whileHover={{ y: -4 }}
                transition={{ duration: 0.5, delay: idx * 0.1 }}
                className="p-4 sm:p-6 rounded-2xl bg-white border border-slate-200/80 shadow-sm space-y-2 sm:space-y-3"
              >
                <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-indigo-50 text-indigo-600 border border-indigo-200/60 font-display font-extrabold text-sm flex items-center justify-center">
                  {step.step}
                </div>
                <h4 className="text-sm sm:text-base font-display font-bold text-slate-900">{step.title}</h4>
                <p className="text-xs text-slate-600 font-normal leading-relaxed">
                  {step.description}
                </p>
              </motion.div>
